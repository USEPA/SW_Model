...
# Week assignments
week_assignments <- read_feather('data/week_assignments.feather')

# Projection - from CyAN
albers_terra <- crs(rast(x = list.files(path = 'data/CyAN', full.names = TRUE, recursive = T)[1]))

# Lakes
conus_lakes <- read_sf('data/OLCI_resolvable_lakes_2022_09_08/OLCI_resolvable_lakes_2022_09_08.shp') %>% st_transform(albers_terra)

# CONUS bound
conus_bound <- read_sf('data/cb_2019_us_state_500k/cb_2019_us_state_500k.shp')  %>% st_transform(albers_terra)

# PRISM air temp 
prism_set_dl_dir("data/PRISM/atemp")
atemp_files <- file.path('data', 'PRISM', 'atemp', prism_archive_ls(),paste0(prism_archive_ls(),'.bil'))
length(atemp_files)
...
mean_atemp %>%
  arrange(COMID,date) %>%
  group_by(COMID) %>%
  mutate(atemp_lag = lag(daily_atemp,1),
         mean_30day = rollmean(atemp_lag,30, fill = NA, align = 'right')) -> output

write_feather(output,'data/30_day_atemp.feather', compression = 'zstd', compression_level = 22)
