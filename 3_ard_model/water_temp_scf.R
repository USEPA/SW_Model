...
#Choose whether to filter scenes with clouds or not
filterClouds = TRUE
...
if (!filterClouds) {
  ggsave('atmos_outputs/ard_partial_plot.jpg', pp_fig,  height = 9, width = 6, units = 'in', dpi = 600, bg = 'white')
}
if (filterClouds) {
  ggsave('atmos_outputs/ard_partial_plot_noclouds.jpg', pp_fig,  height = 9, width = 6, units = 'in', dpi = 600, bg = 'white')
}
...
if (!filterClouds) {
  write_csv(output_for_feather, 'atmos_outputs/ard_oob_preds.csv')
}
if (filterClouds) {
  arrow::write_feather(output_for_feather, 'atmos_outputs/ard_oob_preds_noclouds.feather', compression = 'zstd', compression_level = 22)
}
...
#R squared of whole thing 
r2(rf_model$predicted,training$TEMPERATURE)

if (!filterClouds) {
  write_csv(validation, 'atmos_outputs/ard_validation.csv')
}
if (filterClouds) {
  write_feather(validation, 'atmos_outputs/ard_validation_noclouds.feather', compression = 'zstd', compression_level = 22)
}
...
if (!filterClouds) {
  write_csv(predict_for_csv, 'atmos_outputs/ard_2022_preds.csv')
}
if (filterClouds) {
  write_csv(predict_for_csv, 'atmos_outputs/ard_2022_preds_noclouds.csv')
}
